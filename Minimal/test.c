...
oxy mesure(absorp myAbsorp, param_mesure* ech, oxy myOxy1){
    ...
    if(myAbsorp.acr>=0 && ech->valeur_pres<0){
        ech->passageParZero=1;
        if((myAbsorp.acr>=0 && ech->valeur_pres<0) && ech->passageParZero==1){
            ech->periode_complete=1;
        }
    }
    ech->valeur_pres = myAbsorp.acr;

    

    
    if (ech->periode_complete==1){//On vérifie que la courbe jusque là dans les négatifs dépasse une deuxième fois la valeur 0 (période complète)
...
void init_mesure(param_mesure* ech){//On initialise les valeurs de la structure de mesure
    ...
    ech->Dcr=2030;
    ech->Dcir=2030;
    ech->premierePeriode=0;
    ech->passageParZero=0;
    ech->periode=0;
    ech->poulsPeriod1=0;
    ech->SpO2Period1=0;
    ech->PtP_ACr_prec=0;
